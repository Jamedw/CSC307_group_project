...
app.listen(process.env.PORT || port, () => {
  console.log(`backend app is listening`);
});
  ...
  if (!userId || !communityId || !postTitle || !postContent) {
    ...
    if (!userRes || !communityRes) {
      ...
      if (postTtlExists) {
        res
          .status(409)
          .send('Post with this title already exists in the community');
      } else {
...
app.post('/community/follow', authenticateUser, async (req, res) => {
  ...
  if (!userId || !communityId) {
    res
      .status(400)
      .send('follwing a community requires a userId and a communityId');
  } else {
    const userRes = await findUserById(userId);
    ...
    if (!userRes || !commRes) {
      res
        .status(404)
        .send('A user or community with the given ids could not be found');
    }

    let notFollowing = true;

    if (userRes.communityIds.includes(commRes._id)) {
      notFollowing = false;
    }
...
app.post('/community/unfollow', authenticateUser, async (req, res) => {
  ...
  if (!userId || !communityId) {
    res
      .status(400)
      .send('follwing a communit requires a userId and a communityId');
  } else {
    const userRes = await findUserById(userId);
    const commRes = await findCommunityById(communityId);
    if (!userRes || !commRes) {
...
app.get('/communityName/:commName/:postName', async (req, res) => {
  ...
  if (communityRes.length === 0) {
    ...
    if (found === false) {
      ...
      res
        .status(201)
        .send({ comments: commArr, post: postRes, communityId: comm._id });
    }
  }
});
...
app.post('/post/like', authenticateUser, (req, res) => {
  ...
  if (!userId || !postId) {
    ...
    if (!userRes || !postRes) {
      res
        .status(404)
        .send("Couldn't find a user/post with givern userId/postId");
    } else {
    }
  }
});
